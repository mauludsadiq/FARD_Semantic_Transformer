...
D      = 128  # hidden dim (start smaller, scale up)
N_HEAD = 4
...
for ep in range(80):
    ...
    if ep % 10 == 9:
        ...
        if mean_acc > best_mean:
            best_mean = mean_acc
            torch.save(model.state_dict(), "train/model_sentence_best.pt")
        print(f"ep={ep+1:3d} loss={total_loss:.1f} mean={mean_acc:.3f} "
              f"[{' '.join(f'L{i}={a:.2f}' for i,a in enumerate(accs))}]")

# Final benchmark
model.load_state_dict(torch.load("train/model_sentence_best.pt", map_location="cpu"))
model.eval()
print(f"\n{'='*60}")
...
with open("benchmark/stage5c_sentence_llr.json","w") as f:
